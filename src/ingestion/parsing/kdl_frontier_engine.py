# Vendored from run-llama/ParseBench under Apache-2.0. The ParseBench wrapper
# is excluded; AXIOM's scheduling and ParsedData adapter live in kdl.py.
...
def _nano_payload(
    ...
    payload = {
        ...
        "temperature": 0.0,
        "max_tokens": int(
            (max_output_tokens or {}).get(
                stage,
                os.getenv(
                    _NANO_MAX_TOKENS_ENV[stage][0],
                    str(_NANO_MAX_TOKENS_ENV[stage][1]),
                ),
            )
        ),
    }
...
class _NanoEngine:
        ...
        *,
        ...
        async with httpx.AsyncClient(timeout=self._timeout_s) as client:
            pages = await asyncio.gather(
                *(
                    parse_page(client, image, page_no)
    ...
    async def _parse_page(
        self,
        client: httpx.AsyncClient,
        layout_semaphore: asyncio.Semaphore,
