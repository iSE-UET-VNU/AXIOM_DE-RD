"""Index record builders for parsed and enriched document data."""
...
def build_index_records(
    ...
    for data in enriched_data:
        ...
        document = document_from_enriched_data(data)
...
def document_from_enriched_data(data: EnrichedData) -> DocumentView:
    """Build the temporary indexing view directly from enriched rows."""
...
def build_document_index_record(document: DocumentView, chunks: list[TextChunk]) -> IndexRecord:
...
def build_table_index_records(document: DocumentView) -> list[IndexRecord]:
    records: list[IndexRecord] = []
...
def build_image_index_records(document: DocumentView) -> list[IndexRecord]:
    records: list[IndexRecord] = []
