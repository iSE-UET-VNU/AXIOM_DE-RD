...
def write_processed_artifacts(
    ...
    parsing_results = _parsing_result_records(state)
    paths = {
        ...
        "parsing_results": store.write_jsonl("parsing_results.jsonl", parsing_results),
    }
...
def run(
    ...
    paths: dict[str, Path] = {
        **write_processed_artifacts(state, processed_dir or output_dir, project_root),
        ...
        "index_quality_report": output_reports_store.write_json(
            "index_quality_report.json",
            state.index_quality_report,
        ),
...
def _processed_manifest(
    ...
    document_config = (
        parser_config.get("document") if isinstance(parser_config.get("document"), dict) else {}
    )
    uses_lift = _uses_lift_parser(parser_config, document_config)
    configured_provider = str(parser_config.get("provider") or "").lower().replace("-", "_")
    effective_document_provider = document_config.get("provider")
    if not effective_document_provider:
        effective_document_provider = (
            "lift_api" if configured_provider in {"lift", "lift_api"} else "deferred"
        )
    schema_path = (
        Path(str(lift_config.get("schema_path")))
        if uses_lift and lift_config.get("schema_path")
        else None
    )
    work_root = None
    if state.work_dir:
        ...
        work_root = work_base / state.run_id / "datalab" if uses_lift else work_base / state.run_id

    parsing_results = _parsing_result_records(state)
    successful_statuses = {"complete", "completed", "success", "succeeded"}
    failed_documents = [
    ...
    empty_content_documents = [
        str(document.get("document_id"))
        for document in state.normalized_documents
        if not _document_has_content(document)
    ]
    missing_assets = sum(
    ...
    for result in parsing_results:
        ...
        object_id = str(result.get("source_object_id") or "")
        status_ids.setdefault(result_status, []).append(object_id)
        backend = str(result.get("backend") or "unknown")
        backend_counts[backend] = backend_counts.get(backend, 0) + 1

    counts_by_status = {
        result_status: len(object_ids)
        for result_status, object_ids in sorted(status_ids.items())
    }
    counts_by_backend = dict(sorted(backend_counts.items()))
    parsed_object_ids = {
        str(result.get("source_object_id"))
        for result in parsing_results
        if result.get("source_object_id")
    }
    missing_result_object_ids = [
        str(data_object.object_id)
        for data_object in state.data_objects
        if str(data_object.object_id) not in parsed_object_ids
    ]
    non_success_result_ids = [
        object_id
        for result_status, object_ids in status_ids.items()
    ...
    if (
        non_success_result_ids
        or missing_result_object_ids
        or (not parsing_results and failed_documents)
        or empty_content_documents
        or missing_assets
        or state.errors
    ):
        status = "partial"
    ...
    contracts = {
        ...
        "parsing_results": _first_contract_version(parsing_results),
    }
    return {
        "contract_version": "processed-manifest-v2",
        ...
        "parser": {
            ...
            "document_provider": effective_document_provider,
            "mode": lift_config.get("mode") if uses_lift else None,
            "extract_images": lift_config.get("extract_images") if uses_lift else None,
            "save_raw_outputs": lift_config.get("save_raw_outputs") if uses_lift else None,
        ...
        "artifacts": artifacts,
        "parsing": {
            "result_count": len(parsing_results),
            "counts_by_backend": counts_by_backend,
            "counts_by_status": counts_by_status,
            "deferred_object_ids": status_ids.get("deferred", []),
            "unsupported_object_ids": status_ids.get("unsupported", []),
            "failed_object_ids": status_ids.get("failed", []),
            "missing_result_object_ids": missing_result_object_ids,
        },
        "quality": {
            "failed_document_ids": failed_documents,
            "empty_text_document_ids": empty_text_documents,
            "empty_content_document_ids": empty_content_documents,
...
def _parsing_result_records(state: PipelineState) -> list[dict[str, Any]]:
    records: list[dict[str, Any]] = []
    for result in getattr(state, "parse_results", None) or []:
        backend = _result_value(result, "backend")
        route = _result_value(result, "route") or backend
        status = _enum_value(_result_value(result, "status"))
        error = _result_value(result, "error")
        records.append(
            {
                "contract_version": "parse-result-v1",
                "source_object_id": str(_result_value(result, "source_object_id") or ""),
...
def _document_has_content(document: dict[str, Any]) -> bool:
    quality = document.get("quality") if isinstance(document.get("quality"), dict) else {}
    if "has_content" in quality:
        return bool(quality.get("has_content"))
    if quality.get("has_text"):
        return True
    counts = (
        document.get("component_counts")
        if isinstance(document.get("component_counts"), dict)
        else {}
    )
    return bool(int(counts.get("tables") or 0) or int(counts.get("images") or 0))


def _uses_lift_parser(
    parser_config: dict[str, Any],
    document_config: dict[str, Any],
) -> bool:
    providers = {
        str(parser_config.get("provider") or "").lower().replace("-", "_"),
        str(document_config.get("provider") or "").lower().replace("-", "_"),
    }
    return bool(providers & {"lift", "lift_api"})
...
def _pipeline_state_manifest(state: PipelineState) -> dict[str, Any]:
    return {
        ...
        "counts": {
            ...
            "parse_results": len(getattr(state, "parse_results", None) or []),
...
def _to_jsonable(value: Any, project_root: str | Path | None = None) -> Any:
    ...
    if isinstance(value, Enum):
        return _to_jsonable(value.value, project_root)
