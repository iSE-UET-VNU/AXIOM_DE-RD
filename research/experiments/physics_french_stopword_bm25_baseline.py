"""Fixed French function-word removal for the PDF-inspector BM25 index."""
...
DEFAULT_OUTPUT = DEFAULT_ROOT / "results/physics_french_stopword_bm25_baseline"
...
STOPWORDS = frozenset("a à au aux avec ce ceci cela cette ces dans de des du elle en et est étaient être il ils la le les leur leurs lui mais me même ne nos notre nous on par pas pour que quel quelle quelles quels qui se ses son sont sur ta te tes ton tu un une vos votre vous y d l qu c s".split())


def _analyze_no_stopwords(text: str) -> list[str]:
    return [token for token in analyze(text) if token not in STOPWORDS]
...
def _run(index: BM25Index, pages: list[str], queries: list[str], qids: list[str]) -> dict[str, list[dict[str, Any]]]:
    ...
    for query, qid in zip(queries, qids):
        ...
        result[qid] = [{"chunk_id": pages[position], "doc_id": pages[position], "score": float(score), "rank": rank} for rank, (position, score) in enumerate(hits, 1)]
    return result
...
def main() -> None:
    ...
    index = BM25Index(analyzer_name="no_french_stopwords")
    from src.retrieval import sparse as sparse_module
    sparse_module.ANALYZERS["no_french_stopwords"] = _analyze_no_stopwords
    index.build([{"chunk_id": page, "doc_id": page, "text": text} for page, text in zip(pages, texts)])
    stopword_run = _run(index, pages, queries, qids)
    plain_run = _load_run(BM25_PATH)
    runs = {"plain_pdf_inspector_bm25": plain_run, "french_function_words_removed": stopword_run}
    ...
    report = {"dataset": "vidore_v3/physics", "queries": len(qids), "stopword_count": len(STOPWORDS), "methods": {name: {"page_recall@10": value["page_recall@10"], "ndcg@10": value["ndcg@10"], "file_recall@3": value["file_metrics_by_k"]["3"]["file_recall"]} for name, value in metrics.items()}, "best_full_set": best_full, "oof": {"metrics": oof_metric, "folds": selections}, "notes": ["The removed list is a fixed French function-word list; scientific terms, numbers and units remain indexable.", "OOF selection only chooses between plain and stopword-normalized BM25.", "All results use full page IDs and canonical derived metrics."]}
    (args.output_dir / "report.json").write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    lines = ["# Physics French stopword BM25 baseline", "", "| Method | Page recall@10 | nDCG@10 | File recall@3 |", "|---|---:|---:|---:|"]
