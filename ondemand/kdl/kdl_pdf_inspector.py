...
from .pdf_inspector import PdfInspectorClassifier, PdfInspectorRegionExtractor
...
class _PdfInspectorTextRouter:
    ...
    def __init__(self, classifier: PdfInspectorClassifier, extractor: PdfInspectorRegionExtractor) -> None:
        self._classifier = classifier
        self._extractor = extractor

    def prepare_document(self, source_path: Path) -> _DocumentRoutingContext:
        if source_path.suffix.lower() != '.pdf':
            return _DocumentRoutingContext(source_path=source_path, pdf_type='non_pdf', confidence=0.0, pages_needing_ocr=frozenset({0}))
        ...
        try:
            ...
            logger.warning('pdf-inspector classification failed for %s; using full KDL: %s', source_path, exc)
            return _DocumentRoutingContext(source_path=source_path, pdf_type='classification_error', confidence=0.0, pages_needing_ocr=frozenset(), classification_latency_ms=latency_ms, classification_error=f'{type(exc).__name__}: {exc}')
        ...
        if enabled:
            try:
                ...
                logger.warning('Could not read PDF page dimensions for %s; using full KDL: %s', source_path, exc)
        return _DocumentRoutingContext(source_path=source_path, pdf_type=classification.pdf_type, confidence=classification.confidence, pages_needing_ocr=classification.pages_needing_ocr, page_dimensions=dimensions, native_routing_enabled=enabled, classification_latency_ms=classification.latency_ms, classification_error=dimension_error)

    async def route_text_regions(self, context: _DocumentRoutingContext | None, page_number: int, text_bucket: list[dict[str, Any]]) -> set[int]:
        if context is None:
            return set()
        stats = context.stats
        ...
        for bucket_index, element in enumerate(text_bucket):
            ...
            boxes.append([x1 * page_width, y1 * page_height, x2 * page_width, y2 * page_height])
            valid_candidates.append((bucket_index, element))
        if not boxes:
            return set()
        extract_started = perf_counter()
        try:
            regions = await asyncio.to_thread(self._extractor.extract, context.source_path, page_index, boxes)
        except Exception as exc:
            stats.region_extraction_latency_ms += (perf_counter() - extract_started) * 1000.0
            stats.kdl_text_fallback_regions += len(valid_candidates)
            stats.fallback_reasons['region_extraction_error'] += len(valid_candidates)
            logger.warning('pdf-inspector region extraction failed for %s page %d; using KDL: %s', context.source_path, page_number, exc)
            return set()
        stats.region_extraction_latency_ms += (perf_counter() - extract_started) * 1000.0
        if len(regions) != len(valid_candidates):
            ...
            return set()
        routed: set[int] = set()
        for (bucket_index, element), region in zip(valid_candidates, regions, strict=True):
    ...
    async def route_document_text_regions(self, context: _DocumentRoutingContext | None, page_buckets: dict[int, list[dict[str, Any]]]) -> dict[int, set[int]]:
        routed_by_page = {page: set() for page in page_buckets}
        if context is None:
            return routed_by_page
        stats = context.stats
        pending: list[tuple[int, list[tuple[int, dict[str, Any]]], list[list[float]]]] = []
        ...
        try:
            if hasattr(self._extractor, 'extract_pages'):
                page_results = await asyncio.to_thread(self._extractor.extract_pages, context.source_path, [(page - 1, boxes) for page, _, boxes in pending])
            else:
                page_results = await asyncio.to_thread(lambda: [self._extractor.extract(context.source_path, page - 1, boxes) for page, _, boxes in pending])
        except Exception as exc:
            stats.region_extraction_latency_ms += (perf_counter() - extract_started) * 1000.0
            ...
            logger.warning('pdf-inspector document region extraction failed for %s; using KDL: %s', context.source_path, exc)
            return routed_by_page
        stats.region_extraction_latency_ms += (perf_counter() - extract_started) * 1000.0
        if len(page_results) != len(pending):
            ...
            return routed_by_page
        for (page_number, candidates, _), regions in zip(pending, page_results, strict=True):
            ...
            for (bucket_index, element), region in zip(candidates, regions, strict=True):
    ...
    def routing_metadata(self, context: _DocumentRoutingContext | None) -> dict[str, Any]:
        if context is None:
            return {'pdf_type': 'routing_context_missing', 'classification_confidence': 0.0, 'pages_needing_ocr': [], 'text_candidates': 0, 'native_text_regions': 0, 'kdl_text_fallback_regions': 0, 'classification_latency_ms': 0.0, 'region_extraction_latency_ms': 0.0}
        stats = context.stats
        metadata: dict[str, Any] = {'pdf_type': context.pdf_type, 'classification_confidence': context.confidence, 'pages_needing_ocr': sorted(context.pages_needing_ocr), 'text_candidates': stats.text_candidates, 'native_text_regions': stats.native_text_regions, 'kdl_text_fallback_regions': stats.kdl_text_fallback_regions, 'classification_latency_ms': round(context.classification_latency_ms, 3), 'region_extraction_latency_ms': round(stats.region_extraction_latency_ms, 3)}
        if stats.fallback_reasons:
            metadata['text_routing_fallback_reasons'] = dict(sorted(stats.fallback_reasons.items()))
...
class KdlPdfInspectorProvider(KDLProvider):
    ...
    def __init__(self, config: KDLConfig, *, classifier: PdfInspectorClassifier | None=None, extractor: PdfInspectorRegionExtractor | None=None) -> None:
        resolved_classifier = classifier or PdfInspectorClassifier()
        resolved_extractor = extractor or PdfInspectorRegionExtractor()
        super().__init__(config, text_router=_PdfInspectorTextRouter(resolved_classifier, resolved_extractor))

def _pdf_page_dimensions(path: Path) -> tuple[tuple[float, float], ...]:
    import fitz
    with fitz.open(str(path)) as document:
        return tuple(((float(page.rect.width), float(page.rect.height)) for page in document))
